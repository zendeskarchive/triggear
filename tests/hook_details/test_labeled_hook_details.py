...
class TestLabeledHookDetails:
    ...
    async def test__get_query(self):
        assert LabeledHookDetails('repo', 'master', '123321', 'custom').get_query() == {'repository': 'repo', 'label': 'custom'}
    ...
    async def test__setup_final_params(self):
        ...
        LabeledHookDetails('repo', 'master', '123321', 'custom').setup_final_param_values(registration_cursor)
    ...
    async def test__should_trigger(self):
        ...
        assert await LabeledHookDetails('repo', 'master', '123321', 'custom').should_trigger(registration_cursor, github_client)
        ...
        assert not await LabeledHookDetails('repo', 'master', '123321', 'custom').should_trigger(registration_cursor, github_client)
    ...
    async def test__get_event_type(self):
        assert LabeledHookDetails('repo', 'master', '123321', 'custom').get_event_type() == EventType.PR_LABELED
